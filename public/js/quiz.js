function buildQuiz(quizdata, container='#{{$id}}')
{
    var quiz=$(container);
    ...
    <button class="m-1 btn btn-success mx-auto btndone" onclick="quizDone('#{{$id}}');">Завершить</button>
    `;
    ...
    question_template['radio']=function(question){ return `
            <div class="screen question radio" id="${question.name}" data-name="${question.name}" data-title="${question.title}" data-type="${question.type}">
                <h4 class="title text-center">${question.title}</h4>
                ${question.describe ? `<div class="describe">${question.describe}</div>` : ''}
            </div>
        `;}

    question_template['contacts_phone']=function(question){ return `
    ...
    variant_template['radio']=function(variant){ return `
            <div class="item radio" data-value="${variant.value}">
                ${variant.caption}
            </div>
        `;}

    variant_template['check']=function(variant){ return `
            <div class="item check" data-value="${variant.value}">
                ${variant.caption}
            </div>
        `;}        
    ...
    $.each(quizdata.questions, function(qi, question)
    {
        var question_item=question_template[question.type](question);
        question_item=$(question_item).appendTo(questions_container);
        $.each(question.variants, function(vi, variant){
            if(!variant_template[question.type]) return false;
            var variant_item=$(variant_template[question.type](variant));
            if(variant.break) 
            {
                $(variant_item).data('break', variant.break);
                $(variant_item).data('break-message', variant.break_message);
            }
