        ...
        <div class="container">
            ...
            <x-form method="GET" id="filter">
                <div class="form-group">
                    <x-form-select name="course_id" onchange="$('#filter').submit()" label="Курс">
                    ...
                    <x-form-select name="module_id" onchange="$('#filter').submit()" label="Модуль">
                        <option value="">Все</option>
                    ...
                    <x-form-select name="lesson_id" onchange="$('#filter').submit()" label="Занятие">
                        <option value="">Все</option>
                        @foreach ($lessons as $lesson)
                            <option value="{{ $lesson->lesson_id }}"
                                {{ request()->lesson_id == $lesson->lesson_id ? 'selected' : '' }}>
                                {{ $lesson->lesson_caption }} / {{ $lesson->module->module_caption }} /
                                {{ $lesson->module->course->course_caption }}
                            </option>
                        @endforeach
                    </x-form-select>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @component('component.modal', ['id' => 'lesson_answer_quiz_' . $answer->answer_id, 'title'
                                    => $students->firstWhere('id', $answer->user_id)->name])
                                    @component('component.quiz_answer', ['quiz' => $answer->answer_quiz, 'lesson_id' =>
                                        $answer->lesson_id, 'user_id'=>$answer->user_id])
                                    @endcomponent
                                @endcomponent
                            </td>
                        </tr>
